"""Minimal cross-library repro for bug_143 (divide/generic)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["jax", "keras", "mxnet", "paddle", "tensorflow", "torch"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, Infinity, -Infinity], [-0.0, 0.0, 0.0]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[-Infinity, Infinity, -Infinity], [-0.0, 0.0, 0.0]]}}''')
APIS = json.loads(r'''{"jax": "jax.numpy.divide", "mindspore": "mindspore.numpy.divide"}''')

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.numpy.divide'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.0, -0.0, 1.0]], dtype=np.float32), LIB, QNAME), tensor(np.array([[0.0, 1.401298464324817e-45, 1.0], [float("inf"), 1.0000000031710769e-30, 4.0]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.numpy.divide'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.0, -0.0, 1.0]], dtype=np.float32), LIB, QNAME), tensor(np.array([[0.0, 1.401298464324817e-45, 1.0], [float("inf"), 1.0000000031710769e-30, 4.0]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
