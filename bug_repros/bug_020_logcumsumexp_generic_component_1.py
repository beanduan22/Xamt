"""Minimal cross-library repro for bug_020 (logcumsumexp/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["mindspore", "torch"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, NaN], [-0.3630312979221344, 2.7054264545440674, 2.705869436264038]]}''')
WRONG = json.loads(r'''{"paddle": {"dtype": "float32", "shape": [2, 3], "value": [[-3.4028234663852886e+38, Infinity, Infinity], [-0.3630312979221344, 2.7054264545440674, 2.705869436264038]]}}''')
APIS = json.loads(r'''{"paddle": "paddle.logcumsumexp", "torch": "torch.logcumsumexp"}''')

def run_torch():
    LIB = 'torch'
    QNAME = 'torch.logcumsumexp'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.3630312979221344, 2.657818078994751, -5.01616907119751]], dtype=np.float32), LIB, QNAME), 1)
    return encode(normalize(result))

def run_paddle():
    LIB = 'paddle'
    QNAME = 'paddle.logcumsumexp'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.3630312979221344, 2.657818078994751, -5.01616907119751]], dtype=np.float32), LIB, QNAME), axis=1)
    return encode(normalize(result))
