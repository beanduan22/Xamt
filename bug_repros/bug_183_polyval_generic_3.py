"""Minimal cross-library repro for bug_183 (polyval/generic/3)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [3], "value": [NaN, NaN, 1.0]}''')
WRONG = json.loads(r'''{"tensorflow": {"dtype": "float32", "shape": [3], "value": [NaN, Infinity, 1.0]}}''')
APIS = json.loads(r'''{"jax": "jax.numpy.polyval", "tensorflow": "tensorflow.math.polyval"}''')

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.numpy.polyval'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([2.0, 1.0], dtype=np.float32), LIB, QNAME), tensor(np.array([float("nan"), float("inf"), -0.0], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_tensorflow():
    LIB = 'tensorflow'
    QNAME = 'tensorflow.math.polyval'
    fn = resolve(QNAME)
    result = fn([2.0, 1.0], tensor(np.array([float("nan"), float("inf"), -0.0], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
