"""Minimal cross-library repro for bug_055 (count_nonzero/reduction/component/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "int64", "shape": [], "value": 4}''')
WRONG = json.loads(r'''{"jax": {"dtype": "int32", "shape": [], "value": 3}, "keras": {"dtype": "int32", "shape": [], "value": 3}, "mindspore": {"dtype": "int32", "shape": [], "value": 3}, "tensorflow": {"dtype": "int64", "shape": [], "value": 3}}''')
APIS = json.loads(r'''{"jax": "jax.numpy.count_nonzero", "keras": "keras.ops.count_nonzero", "mindspore": "mindspore.numpy.count_nonzero", "numpy": "numpy.count_nonzero", "tensorflow": "tensorflow.math.count_nonzero"}''')

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.count_nonzero'
    fn = resolve(QNAME)
...
def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.numpy.count_nonzero'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[-0.0, 0.0, 1.401298464324817e-45], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
...
def run_tensorflow():
    LIB = 'tensorflow'
    QNAME = 'tensorflow.math.count_nonzero'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[-0.0, 0.0, 1.401298464324817e-45], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
