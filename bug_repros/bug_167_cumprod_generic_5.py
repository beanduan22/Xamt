"""Minimal cross-library repro for bug_167 (cumprod/generic/5)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["numpy", "paddle"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[9.999999680285692e+37, -Infinity, -Infinity], [-1.401298464324817e-45, -0.0, 0.0]]}''')
WRONG = json.loads(r'''{"tensorflow": {"dtype": "float32", "shape": [2, 3], "value": [[9.999999680285692e+37, -Infinity, NaN], [-0.0, -0.0, 0.0]]}}''')
APIS = json.loads(r'''{"numpy": "numpy.cumulative_prod", "tensorflow": "tensorflow.math.cumprod"}''')

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.cumulative_prod'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[9.999999680285692e+37, -9.999999680285692e+37, 1.401298464324817e-45], [-1.401298464324817e-45, 0.0, -0.0]], dtype=np.float32), LIB, QNAME), axis=1)
    return encode(normalize(result))

def run_tensorflow():
    LIB = 'tensorflow'
    QNAME = 'tensorflow.math.cumprod'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[9.999999680285692e+37, -9.999999680285692e+37, 1.401298464324817e-45], [-1.401298464324817e-45, 0.0, -0.0]], dtype=np.float32), LIB, QNAME), axis=1)
    return encode(normalize(result))
