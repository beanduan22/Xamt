"""Minimal cross-library repro for bug_064 (mish/generic/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, Infinity, NaN], [-0.30340149998664856, 0.8650984168052673, 6.000003622830263e-07]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, NaN], [-0.30340102314949036, 0.865098237991333, 6.000003054396075e-07]]}}''')
...
def run_tensorflow():
    ...
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
