"""Minimal cross-library repro for bug_107 (gammaln/generic/component/1)."""
import importlib
import json
import os
...
WRONG = json.loads(r'''{"jax": {"dtype": "float32", "shape": [2, 3], "value": [[Infinity, Infinity, Infinity], [Infinity, 4.76837158203125e-07, 13.815508842468262]]}}''')
APIS = json.loads(r'''{"jax": "jax.scipy.special.gammaln", "torch": "torch.special.gammaln"}''')

def run_torch():
    LIB = 'torch'
    QNAME = 'torch.special.gammaln'
    fn = resolve(QNAME)
...
def run_jax():
    LIB = 'jax'
    QNAME = 'jax.scipy.special.gammaln'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[-0.0, 0.0, 1.401298464324817e-45], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
