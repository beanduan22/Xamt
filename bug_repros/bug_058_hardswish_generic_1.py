"""Minimal cross-library repro for bug_058 (hardswish/generic/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, Infinity, NaN], [-0.06668020784854889, 0.45176005363464355, 0.29167962074279785]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, Infinity, 0.0], [-0.06668020784854889, 0.45176002383232117, 0.29167965054512024]]}}''')
...
def run_tensorflow():
    ...
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.13988281786441803, 0.727231502532959, 0.5000194311141968]], dtype=np.float32), LIB, QNAME))
