"""Minimal cross-library repro for bug_164 (std_mean/reduction/component/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''[{"dtype": "float32", "shape": [], "value": 0.4945068359375}, {"dtype": "float32", "shape": [], "value": -0.4608665108680725}]''')
WRONG = json.loads(r'''{"mindspore": [{"dtype": "float32", "shape": [], "value": 0.4514209032058716}, {"dtype": "float32", "shape": [], "value": -0.4608665406703949}]}''')
APIS = json.loads(r'''{"mindspore": "mindspore.ops.function.std_mean", "torch": "torch.std_mean"}''')

def run_torch():
    LIB = 'torch'
    QNAME = 'torch.std_mean'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[9.999999680285692e+37, -9.999999680285692e+37, 1.401298464324817e-45], [-1.401298464324817e-45, 0.0, -0.0]], dtype=np.float32), LIB, QNAME), dim=1, correction=0)
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.function.std_mean'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[9.999999680285692e+37, -9.999999680285692e+37, 1.401298464324817e-45], [-1.401298464324817e-45, 0.0, -0.0]], dtype=np.float32), LIB, QNAME), axis=1, ddof=0)
    return encode(normalize(result))
