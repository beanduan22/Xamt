"""Minimal cross-library repro for bug_006 (signbit/generic/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["jax", "keras"]''')
...
APIS = json.loads(r'''{"jax": "jax.numpy.signbit", "mindspore": "mindspore.ops.function.signbit"}''')
...
def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.function.signbit'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[-0.0, 0.0, 1.401298464324817e-45], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
