"""Minimal cross-library repro for bug_001 (clip/generic/3)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["chainer", "jax", "keras"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, 2.0, -1.0], [0.3003249168395996, -1.0, -0.0947190374135971]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[-1.0, 2.0, -1.0], [0.3003249168395996, -1.0, -0.0947190374135971]]}}''')
APIS = json.loads(r'''{"jax": "jax.lax.clamp", "mindspore": "mindspore.ops.clamp"}''')

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.lax.clamp'
    fn = resolve(QNAME)
    result = fn(-1.0, tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.3003249168395996, -1.1990708112716675, -0.0947190374135971]], dtype=np.float32), LIB, QNAME), 2.0)
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.clamp'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.3003249168395996, -1.1990708112716675, -0.0947190374135971]], dtype=np.float32), LIB, QNAME), -1.0, 2.0)
    return encode(normalize(result))
