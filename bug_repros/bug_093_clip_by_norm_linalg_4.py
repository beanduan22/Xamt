"""Minimal cross-library repro for bug_093 (clip_by_norm/linalg/4)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, NaN], [NaN, NaN, NaN]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, Infinity, -Infinity], [-1.0, 1.0, 9.999999974752427e-07]]}}''')
APIS = json.loads(r'''{"mindspore": "mindspore.ops.clip_by_norm", "tensorflow": "tensorflow.clip_by_norm"}''')

def run_tensorflow():
    LIB = 'tensorflow'
    QNAME = 'tensorflow.clip_by_norm'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME), 1.0)
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.clip_by_norm'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME), 1.0)
    return encode(normalize(result))
