"""Minimal cross-library repro for bug_110 (lgamma/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[Infinity, Infinity, 103.2789306640625], [Infinity, 0.0, 13.815509796142578]]}''')
WRONG = json.loads(r'''{"jax": {"dtype": "float32", "shape": [2, 3], "value": [[Infinity, Infinity, Infinity], [Infinity, 4.76837158203125e-07, 13.815508842468262]]}, "mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[Infinity, Infinity, 103.97207641601562], [Infinity, 0.0, 13.815509796142578]]}, "tensorflow": {"dtype": "float32", "shape": [2, 3], "value": [[Infinity, Infinity, 103.97207641601562], [Infinity, 0.0, 13.815509796142578]]}}''')
APIS = json.loads(r'''{"jax": "jax.lax.lgamma", "mindspore": "mindspore.ops.function.lgamma", "tensorflow": "tensorflow.math.lgamma", "torch": "torch.lgamma"}''')
...
def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.function.lgamma'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[-0.0, 0.0, 1.401298464324817e-45], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
