"""Minimal cross-library repro for bug_170 (min/reduction/3)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["chainer", "keras"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [], "value": NaN}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [], "value": -0.0}}''')
APIS = json.loads(r'''{"keras": "keras.ops.amin", "mindspore": "mindspore.mint.amin"}''')

def run_keras():
    LIB = 'keras'
    QNAME = 'keras.ops.amin'
    fn = resolve(QNAME)
