"""Minimal cross-library repro for bug_106 (erfcinv/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[Infinity, Infinity, Infinity], [NaN, -0.0, 3.4589104652404785]]}''')
WRONG = json.loads(r'''{"chainer": {"dtype": "float32", "shape": [2, 3], "value": [[Infinity, Infinity, 10.019834518432617], [NaN, -0.0, 3.4589107036590576]]}}''')
APIS = json.loads(r'''{"chainer": "chainer.functions.erfcinv", "tensorflow": "tensorflow.math.erfcinv"}''')

def run_tensorflow():
    LIB = 'tensorflow'
    QNAME = 'tensorflow.math.erfcinv'
    fn = resolve(QNAME)
...
def run_chainer():
    LIB = 'chainer'
    QNAME = 'chainer.functions.erfcinv'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[-0.0, 0.0, 1.401298464324817e-45], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
