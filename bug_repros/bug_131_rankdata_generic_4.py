"""Minimal cross-library repro for bug_131 (rankdata/generic/4)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float64", "shape": [6], "value": [NaN, NaN, NaN, NaN, NaN, NaN]}''')
WRONG = json.loads(r'''{"jax": {"dtype": "float32", "shape": [6], "value": [6.0, 5.0, 1.0, 3.0, 4.0, 2.0]}}''')
APIS = json.loads(r'''{"jax": "jax.scipy.stats.rankdata", "scipy": "scipy.stats.rankdata"}''')

def run_scipy():
    LIB = 'scipy'
    QNAME = 'scipy.stats.rankdata'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.02075602486729622, 0.2733055651187897, -0.01565474271774292]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.scipy.stats.rankdata'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.02075602486729622, 0.2733055651187897, -0.01565474271774292]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
