"""Minimal cross-library repro for bug_043 (log_softmax/nn/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["chainer", "jax", "keras", "paddle", "scipy", "tensorflow", "torch"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, NaN], [-1.1771713495254517, -0.9787102341651917, -1.1518378257751465]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, -Infinity], [-1.1771693229675293, -0.9787082076072693, -1.1518357992172241]]}, "mxnet": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, -Infinity], [-1.1771693229675293, -0.9787082076072693, -1.1518357992172241]]}}''')
APIS = json.loads(r'''{"mindspore": "mindspore.ops.function.log_softmax", "mxnet": "mxnet.ndarray.log_softmax", "scipy": "scipy.special.log_softmax"}''')

def run_scipy():
    LIB = 'scipy'
    QNAME = 'scipy.special.log_softmax'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.06634701043367386, 0.13211409747600555, -0.04101350158452988]], dtype=np.float32), LIB, QNAME), axis=1)
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.function.log_softmax'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.06634701043367386, 0.13211409747600555, -0.04101350158452988]], dtype=np.float32), LIB, QNAME), axis=1)
    return encode(normalize(result))
...
def run_mxnet():
    LIB = 'mxnet'
    QNAME = 'mxnet.ndarray.log_softmax'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.06634701043367386, 0.13211409747600555, -0.04101350158452988]], dtype=np.float32), LIB, QNAME), axis=1)
    return encode(normalize(result))
