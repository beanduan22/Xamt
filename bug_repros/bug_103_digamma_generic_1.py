"""Minimal cross-library repro for bug_103 (digamma/generic/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["chainer", "mindspore"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[Infinity, -Infinity, -Infinity], [NaN, -0.5772156715393066, -1000000.5625]]}''')
WRONG = json.loads(r'''{"jax": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, NaN], [NaN, -0.5772153735160828, -1000000.5625]]}}''')
APIS = json.loads(r'''{"chainer": "chainer.functions.digamma", "jax": "jax.scipy.special.digamma"}''')

def run_chainer():
    LIB = 'chainer'
    QNAME = 'chainer.functions.digamma'
    fn = resolve(QNAME)
