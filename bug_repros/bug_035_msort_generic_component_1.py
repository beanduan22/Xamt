"""Minimal cross-library repro for bug_035 (msort/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["mxnet", "paddle", "torch"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[-1.0333818197250366, 0.2164539247751236, -Infinity], [NaN, Infinity, 0.7698968052864075]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, 0.2164539247751236, -Infinity], [-1.0333818197250366, Infinity, 0.7698968052864075]]}}''')
APIS = json.loads(r'''{"mindspore": "mindspore.ops.function.msort", "torch": "torch.msort"}''')

def run_torch():
    LIB = 'torch'
    QNAME = 'torch.msort'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-1.0333818197250366, 0.2164539247751236, 0.7698968052864075]], dtype=np.float32), LIB, QNAME))
