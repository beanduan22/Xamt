"""Minimal cross-library repro for bug_042 (hardswish/nn/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["jax", "paddle", "torch"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, Infinity, NaN], [-0.2664388120174408, -0.31546351313591003, 0.16632360219955444]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, Infinity, 0.0], [-0.2664388120174408, -0.31546348333358765, 0.16632358729839325]]}}''')
APIS = json.loads(r'''{"jax": "jax.nn.hard_silu", "mindspore": "mindspore.mint.nn.functional.hardswish"}''')

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.nn.hard_silu'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.692926824092865, -0.902321994304657, 0.3022046387195587]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.mint.nn.functional.hardswish'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.692926824092865, -0.902321994304657, 0.3022046387195587]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
