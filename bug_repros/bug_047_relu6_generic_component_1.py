"""Minimal cross-library repro for bug_047 (relu6/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, 6.0, 0.0], [0.36694541573524475, 0.31849777698516846, 0.21835516393184662]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[0.0, 6.0, 0.0], [0.36694541573524475, 0.31849777698516846, 0.21835516393184662]]}}''')
...
def run_tensorflow():
    ...
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.36694541573524475, 0.31849777698516846, 0.21835516393184662]], dtype=np.float32), LIB, QNAME))
