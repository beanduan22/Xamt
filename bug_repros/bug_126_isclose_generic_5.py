"""Minimal cross-library repro for bug_126 (isclose/generic/5)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["jax", "mindspore", "mxnet", "numpy"]''')
EXPECTED = json.loads(r'''{"dtype": "bool", "shape": [2, 3], "value": [[false, true, true], [true, true, true]]}''')
WRONG = json.loads(r'''{"keras": {"dtype": "bool", "shape": [2, 3], "value": [[false, false, false], [true, true, true]]}}''')
...
def run_numpy():
    ...
    QNAME = 'numpy.isclose'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-2.0675485134124756, 2.2419116497039795, -2.3091959953308105]], dtype=np.float32), LIB, QNAME), tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-2.067547559738159, 2.241912603378296, -2.309195041656494]], dtype=np.float32), LIB, QNAME))
