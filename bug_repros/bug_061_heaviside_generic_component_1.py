"""Minimal cross-library repro for bug_061 (heaviside/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [3], "value": [0.5, 0.5, 0.5]}''')
WRONG = json.loads(r'''{"paddle": {"dtype": "float32", "shape": [3], "value": [0.5, 0.5, 1.0]}, "torch": {"dtype": "float32", "shape": [3], "value": [0.5, 0.5, 1.0]}}''')
APIS = json.loads(r'''{"jax": "jax.numpy.heaviside", "paddle": "paddle.heaviside", "torch": "torch.heaviside"}''')

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.numpy.heaviside'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([-0.0, 0.0, 1.401298464324817e-45], dtype=np.float32), LIB, QNAME), tensor(np.array([0.5, 0.5, 0.5], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_paddle():
    LIB = 'paddle'
    QNAME = 'paddle.heaviside'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([-0.0, 0.0, 1.401298464324817e-45], dtype=np.float32), LIB, QNAME), tensor(np.array([0.5, 0.5, 0.5], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
