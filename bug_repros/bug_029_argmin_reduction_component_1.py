"""Minimal cross-library repro for bug_029 (argmin/reduction/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["chainer", "jax", "mindspore", "mxnet", "numpy", "torch"]''')
EXPECTED = json.loads(r'''{"dtype": "int32", "shape": [2], "value": [0, 0]}''')
WRONG = json.loads(r'''{"keras": {"dtype": "int32", "shape": [2], "value": [2, 0]}, "paddle": {"dtype": "int64", "shape": [2], "value": [2, 0]}, "tensorflow": {"dtype": "int64", "shape": [2], "value": [2, 0]}}''')
APIS = json.loads(r'''{"keras": "keras.ops.argmin", "numpy": "numpy.argmin", "paddle": "paddle.argmin", "tensorflow": "tensorflow.math.argmin"}''')

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.argmin'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-1.8491108417510986, -1.73651921749115, 1.1329257488250732]], dtype=np.float32), LIB, QNAME), axis=1)
    return encode(normalize(result))

def run_keras():
    LIB = 'keras'
    QNAME = 'keras.ops.argmin'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-1.8491108417510986, -1.73651921749115, 1.1329257488250732]], dtype=np.float32), LIB, QNAME), axis=1)
    return encode(normalize(result))
