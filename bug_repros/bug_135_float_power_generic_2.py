"""Minimal cross-library repro for bug_135 (float_power/generic/2)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [3], "value": [0.0, Infinity, Infinity]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float64", "shape": [3], "value": [0.0, Infinity, 3.8938863925217126e+70]}}''')
APIS = json.loads(r'''{"jax": "jax.numpy.float_power", "mindspore": "mindspore.ops.float_power"}''')

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.numpy.float_power'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([-0.0, 0.0, 1.401298464324817e-45], dtype=np.float32), LIB, QNAME), tensor(np.array([1.2607743740081787, -0.7484014630317688, -1.5737998485565186], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.float_power'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([-0.0, 0.0, 1.401298464324817e-45], dtype=np.float32), LIB, QNAME), tensor(np.array([1.2607743740081787, -0.7484014630317688, -1.5737998485565186], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
