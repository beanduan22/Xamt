"""Minimal cross-library repro for bug_026 (argmin/reduction/3)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["jax", "mindspore", "mxnet"]''')
EXPECTED = json.loads(r'''{"dtype": "int32", "shape": [2], "value": [0, 0]}''')
WRONG = json.loads(r'''{"keras": {"dtype": "int32", "shape": [2], "value": [2, 0]}}''')
APIS = json.loads(r'''{"jax": "jax.lax.argmin", "keras": "keras.ops.argmin"}''')

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.lax.argmin'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME), 1, dtype_for(LIB, 'int32'))
...
def run_keras():
    ...
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME), axis=1)
    return encode(normalize(result))
