"""Minimal cross-library repro for bug_137 (fft/fft)."""
import importlib
import json
import os
...
WRONG = json.loads(r'''{"jax": {"dtype": "complex64", "shape": [4], "value": [[NaN, 0.0], [NaN, NaN], [NaN, 0.0], [NaN, NaN]]}, "keras": {"dtype": "complex64", "shape": [4], "value": [[NaN, 0.0], [NaN, NaN], [NaN, 0.0], [NaN, NaN]]}, "tensorflow": {"dtype": "complex64", "shape": [4], "value": [[NaN, 0.0], [NaN, NaN], [NaN, 0.0], [NaN, NaN]]}}''')
APIS = json.loads(r'''{"jax": "jax.numpy.fft.fft", "keras": "keras.ops.fft", "numpy": "numpy.fft.fft", "tensorflow": "tensorflow.signal.fft"}''')
...
def run_keras():
    ...
    QNAME = 'keras.ops.fft'
    fn = resolve(QNAME)
    result = fn((tensor(np.array([float("nan"), float("inf"), -float("inf"), -1.0], dtype=np.float32), LIB, QNAME), tensor(np.array([0.0, 0.0, 0.0, 0.0], dtype=np.float32), LIB, QNAME)))
    return encode(normalize(result))

def run_tensorflow():
    LIB = 'tensorflow'
    QNAME = 'tensorflow.signal.fft'
    fn = resolve(QNAME)
