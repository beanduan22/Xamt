"""Minimal cross-library repro for bug_121 (sgn/generic/component/1)."""
import importlib
import json
import os
...
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[0.0, 0.0, 0.0], [-1.0, 1.0, 1.0]]}}''')
APIS = json.loads(r'''{"mindspore": "mindspore.ops.function.sgn", "torch": "torch.sgn"}''')
...
def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.function.sgn'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.0, -0.0, 1.0]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
