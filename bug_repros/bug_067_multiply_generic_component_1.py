"""Minimal cross-library repro for bug_067 (multiply/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["mindspore", "mxnet", "paddle", "torch"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, -Infinity], [0.35449573397636414, 0.04463260620832443, 1.1511584574463996e-07]]}''')
WRONG = json.loads(r'''{"jax": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, NaN], [0.35449573397636414, 0.04463260620832443, 1.1511584574463996e-07]]}, "keras": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, NaN], [0.35449573397636414, 0.04463260620832443, 1.1511584574463996e-07]]}, "tensorflow": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, NaN], [0.35449573397636414, 0.04463260620832443, 1.1511584574463996e-07]]}}''')
APIS = json.loads(r'''{"jax": "jax.lax.mul", "keras": "keras.ops.multiply", "tensorflow": "tensorflow.math.multiply", "torch": "torch.mul"}''')

def run_torch():
    LIB = 'torch'
    QNAME = 'torch.mul'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.35449573397636414, 0.04463260620832443, 0.11511584371328354]], dtype=np.float32), LIB, QNAME), tensor(np.array([[-0.0, 0.0, 1.401298464324817e-45], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
...
def run_tensorflow():
    LIB = 'tensorflow'
    QNAME = 'tensorflow.math.multiply'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.35449573397636414, 0.04463260620832443, 0.11511584371328354]], dtype=np.float32), LIB, QNAME), tensor(np.array([[-0.0, 0.0, 1.401298464324817e-45], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
