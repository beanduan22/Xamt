"""Minimal cross-library repro for bug_094 (geqrf/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''[{"dtype": "float32", "shape": [2, 3], "value": [[-1.0, 1.0, 9.999999974752427e-07], [-1.0, 0.0, 0.0]]}, {"dtype": "float32", "shape": [2], "value": [1.0, 0.0]}]''')
WRONG = json.loads(r'''{"mindspore": [{"dtype": "float32", "shape": [2, 3], "value": [[1.0, -1.0, -9.999999974752427e-07], [1.0, 0.0, 0.0]]}, {"dtype": "float32", "shape": [2], "value": [1.0, 0.0]}]}''')
APIS = json.loads(r'''{"mindspore": "mindspore.ops.function.geqrf", "torch": "torch.geqrf"}''')
...
def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.function.geqrf'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[-0.0, 0.0, 1.401298464324817e-45], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
