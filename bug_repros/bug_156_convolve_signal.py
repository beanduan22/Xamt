"""Minimal cross-library repro for bug_156 (convolve/signal)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["jax", "numpy", "scipy"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [5], "value": [NaN, NaN, NaN, NaN, -Infinity]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [5], "value": [NaN, NaN, NaN, NaN, NaN]}}''')
APIS = json.loads(r'''{"mindspore": "mindspore.numpy.convolve", "numpy": "numpy.convolve"}''')

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.convolve'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([float("nan"), float("inf"), -0.0], dtype=np.float32), LIB, QNAME), tensor(np.array([1.0, -float("inf"), 0.0], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.numpy.convolve'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([float("nan"), float("inf"), -0.0], dtype=np.float32), LIB, QNAME), tensor(np.array([1.0, -float("inf"), 0.0], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
