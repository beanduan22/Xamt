"""Minimal cross-library repro for bug_185 (reciprocal/generic/2)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["mindspore", "mxnet", "paddle"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[Infinity, 1.0000000694406173e+38, 1.0], [1.0000000751754868e-38, 0.0625, 0.03999999910593033]]}''')
WRONG = json.loads(r'''{"tensorflow": {"dtype": "float32", "shape": [2, 3], "value": [[Infinity, Infinity, 1.0], [0.0, 0.0625, 0.03999999910593033]]}}''')
APIS = json.loads(r'''{"mxnet": "mxnet.numpy.reciprocal", "tensorflow": "tensorflow.math.reciprocal"}''')

def run_mxnet():
    LIB = 'mxnet'
    QNAME = 'mxnet.numpy.reciprocal'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[1.401298464324817e-45, 9.999999350456404e-39, 1.0], [9.999999680285692e+37, 16.0, 25.0]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_tensorflow():
    LIB = 'tensorflow'
    QNAME = 'tensorflow.math.reciprocal'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[1.401298464324817e-45, 9.999999350456404e-39, 1.0], [9.999999680285692e+37, 16.0, 25.0]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
