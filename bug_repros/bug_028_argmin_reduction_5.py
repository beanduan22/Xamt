"""Minimal cross-library repro for bug_028 (argmin/reduction/5)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "int64", "shape": [2], "value": [2, 2]}''')
WRONG = json.loads(r'''{"mxnet": {"dtype": "float32", "shape": [2], "value": [0.0, 2.0]}}''')
APIS = json.loads(r'''{"mxnet": "mxnet.ndarray.argmin", "paddle": "paddle.argmin"}''')

def run_paddle():
    LIB = 'paddle'
    QNAME = 'paddle.argmin'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.1912984848022461, -0.054025549441576004, -0.2519875466823578]], dtype=np.float32), LIB, QNAME), axis=1)
    return encode(normalize(result))

def run_mxnet():
    LIB = 'mxnet'
    QNAME = 'mxnet.ndarray.argmin'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.1912984848022461, -0.054025549441576004, -0.2519875466823578]], dtype=np.float32), LIB, QNAME), axis=1)
    return encode(normalize(result))
