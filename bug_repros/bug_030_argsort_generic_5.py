"""Minimal cross-library repro for bug_030 (argsort/generic/5)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "int64", "shape": [2, 3], "value": [[2, 1, 0], [0, 1, 2]]}''')
WRONG = json.loads(r'''{"tensorflow": {"dtype": "int32", "shape": [2, 3], "value": [[0, 2, 1], [0, 1, 2]]}}''')
APIS = json.loads(r'''{"numpy": "numpy.argsort", "tensorflow": "tensorflow.argsort"}''')

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.argsort'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.46320265531539917, -0.41812050342559814, 0.16320890188217163]], dtype=np.float32), LIB, QNAME))
