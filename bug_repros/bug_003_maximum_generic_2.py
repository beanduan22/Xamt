"""Minimal cross-library repro for bug_003 (maximum/generic/2)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["chainer", "keras", "mxnet"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, Infinity, 1.2976676225662231], [-1.0, 1.0, 0.5523166656494141]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[1.2180202007293701, Infinity, 1.2976676225662231], [-1.0, 1.0, 0.5523166656494141]]}}''')
APIS = json.loads(r'''{"keras": "keras.ops.maximum", "mindspore": "mindspore.ops.function.maximum"}''')

def run_keras():
    LIB = 'keras'
    QNAME = 'keras.ops.maximum'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME), tensor(np.array([[1.2180202007293701, 0.22934786975383759, 1.2976676225662231], [-1.0315568447113037, 0.013808977790176868, 0.5523166656494141]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.function.maximum'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME), tensor(np.array([[1.2180202007293701, 0.22934786975383759, 1.2976676225662231], [-1.0315568447113037, 0.013808977790176868, 0.5523166656494141]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
