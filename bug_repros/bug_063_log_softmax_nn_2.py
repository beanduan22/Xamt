"""Minimal cross-library repro for bug_063 (log_softmax/nn/2)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["chainer", "jax", "keras", "scipy", "tensorflow"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, NaN], [-0.892629861831665, -0.9274938702583313, -1.6353760957717896]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, -Infinity], [-0.8926306962966919, -0.9274947047233582, -1.6353769302368164]]}}''')
APIS = json.loads(r'''{"mindspore": "mindspore.ops.function.log_softmax", "scipy": "scipy.special.log_softmax"}''')

def run_scipy():
    LIB = 'scipy'
    QNAME = 'scipy.special.log_softmax'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.793718695640564, 0.7588546872138977, 0.050972484052181244]], dtype=np.float32), LIB, QNAME), axis=1)
