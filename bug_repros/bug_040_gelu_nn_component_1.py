"""Minimal cross-library repro for bug_040 (gelu/nn/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["jax", "keras", "mindspore", "paddle", "tensorflow"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, Infinity, NaN], [-0.01283420529216528, 0.06086074933409691, -0.02035370096564293]]}''')
WRONG = json.loads(r'''{"torch": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, NaN], [-0.01283420529216528, 0.06086074933409691, -0.02035370096564293]]}}''')
APIS = json.loads(r'''{"jax": "jax.nn.gelu", "torch": "torch.nn.functional.gelu"}''')

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.nn.gelu'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.026216749101877213, 0.11177391558885574, -0.04212268441915512]], dtype=np.float32), LIB, QNAME), approximate=0)
    return encode(normalize(result))

def run_torch():
    LIB = 'torch'
    QNAME = 'torch.nn.functional.gelu'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.026216749101877213, 0.11177391558885574, -0.04212268441915512]], dtype=np.float32), LIB, QNAME), approximate='none')
    return encode(normalize(result))
