"""Minimal cross-library repro for bug_014 (nanstd/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["jax", "keras", "numpy"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [], "value": NaN}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [], "value": 0.0}}''')
APIS = json.loads(r'''{"mindspore": "mindspore.numpy.nanstd", "numpy": "numpy.nanstd"}''')

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.nanstd'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-1.3851065635681152, -0.3388097882270813, -0.3164989650249481]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.numpy.nanstd'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-1.3851065635681152, -0.3388097882270813, -0.3164989650249481]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
