"""Minimal cross-library repro for bug_046 (relu/nn/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["chainer", "keras", "mxnet", "tensorflow", "torch"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, Infinity, 0.0], [1.2985645532608032, 0.0, 0.0]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[0.0, Infinity, 0.0], [1.2985645532608032, 0.0, 0.0]]}, "paddle": {"dtype": "float32", "shape": [2, 3], "value": [[0.0, Infinity, 0.0], [1.2985645532608032, 0.0, 0.0]]}}''')
APIS = json.loads(r'''{"mindspore": "mindspore.mint.nn.functional.relu", "paddle": "paddle.nn.functional.relu", "torch": "torch.nn.functional.relu"}''')

def run_torch():
    LIB = 'torch'
    QNAME = 'torch.nn.functional.relu'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [1.2985645532608032, -0.6106696128845215, -2.752192258834839]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.mint.nn.functional.relu'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [1.2985645532608032, -0.6106696128845215, -2.752192258834839]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
...
def run_paddle():
    LIB = 'paddle'
    QNAME = 'paddle.nn.functional.relu'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [1.2985645532608032, -0.6106696128845215, -2.752192258834839]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
