"""Minimal cross-library repro for bug_129 (quantile/generic/7)."""
import importlib
import json
import os
...
WRONG = json.loads(r'''{"mxnet": {"dtype": "float32", "shape": [], "value": 0.23651906847953796}}''')
APIS = json.loads(r'''{"jax": "jax.numpy.quantile", "mxnet": "mxnet.numpy.quantile"}''')

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.numpy.quantile'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.14062969386577606, 0.33240845799446106, 0.404375821352005]], dtype=np.float32), LIB, QNAME), 0.5)
