"""Minimal cross-library repro for bug_130 (quantile/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["jax", "keras", "numpy", "paddle", "torch"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [], "value": NaN}''')
WRONG = json.loads(r'''{"mxnet": {"dtype": "float32", "shape": [], "value": -0.025599300861358643}}''')
APIS = json.loads(r'''{"mxnet": "mxnet.numpy.quantile", "numpy": "numpy.quantile"}''')

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.quantile'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.9657704830169678, -1.016969084739685, 9.8560791015625]], dtype=np.float32), LIB, QNAME), 0.5)
    return encode(normalize(result))

def run_mxnet():
    LIB = 'mxnet'
    QNAME = 'mxnet.numpy.quantile'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.9657704830169678, -1.016969084739685, 9.8560791015625]], dtype=np.float32), LIB, QNAME), 0.5)
    return encode(normalize(result))
