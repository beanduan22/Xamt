"""Minimal cross-library repro for bug_142 (argpartition/generic)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "int64", "shape": [2, 3], "value": [[2, 1, 0], [0, 2, 1]]}''')
WRONG = json.loads(r'''{"jax": {"dtype": "int32", "shape": [2, 3], "value": [[0, 2, 1], [0, 2, 1]]}}''')
APIS = json.loads(r'''{"jax": "jax.numpy.argpartition", "numpy": "numpy.argpartition"}''')

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.argpartition'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.0, -0.0, 1.0]], dtype=np.float32), LIB, QNAME), 1, axis=1)
    return encode(normalize(result))

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.numpy.argpartition'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.0, -0.0, 1.0]], dtype=np.float32), LIB, QNAME), 1, axis=1)
    return encode(normalize(result))
