"""Minimal cross-library repro for bug_147 (logaddexp2/generic)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["keras", "torch"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [3], "value": [1.0000000200408773e+20, 1.0000000200408773e+20, 88.0]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [3], "value": [Infinity, Infinity, 88.0]}}''')
APIS = json.loads(r'''{"mindspore": "mindspore.numpy.logaddexp2", "torch": "torch.logaddexp2"}''')

def run_torch():
    LIB = 'torch'
    QNAME = 'torch.logaddexp2'
    fn = resolve(QNAME)
...
def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.numpy.logaddexp2'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([9.999999680285692e+37, -9.999999680285692e+37, 1.401298464324817e-45], dtype=np.float32), LIB, QNAME), tensor(np.array([9.999999350456404e-39, -9.999999350456404e-39, -0.0], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
