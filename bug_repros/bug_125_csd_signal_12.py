"""Minimal cross-library repro for bug_125 (csd/signal/12)."""
import importlib
import json
import os
...
def run_scipy():
    ...
    fn = resolve(QNAME)
    result = fn(tensor(np.array([float("nan"), float("inf"), -float("inf"), -0.03717648983001709], dtype=np.float32), LIB, QNAME), tensor(np.array([float("nan"), float("inf"), -float("inf"), -0.03717648983001709], dtype=np.float32), LIB, QNAME), nperseg=4)
