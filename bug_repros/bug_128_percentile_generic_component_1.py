"""Minimal cross-library repro for bug_128 (percentile/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["jax", "numpy"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [], "value": NaN}''')
WRONG = json.loads(r'''{"mxnet": {"dtype": "float32", "shape": [], "value": -0.7107062339782715}}''')
APIS = json.loads(r'''{"mxnet": "mxnet.numpy.percentile", "numpy": "numpy.percentile"}''')

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.percentile'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.720385730266571, -0.9347317814826965, -0.48668065667152405]], dtype=np.float32), LIB, QNAME), 50.0)
    return encode(normalize(result))

def run_mxnet():
    LIB = 'mxnet'
    QNAME = 'mxnet.numpy.percentile'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.720385730266571, -0.9347317814826965, -0.48668065667152405]], dtype=np.float32), LIB, QNAME), 50.0)
    return encode(normalize(result))
