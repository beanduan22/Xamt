"""Minimal cross-library repro for bug_152 (hfft/fft)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [6], "value": [-1.0000000200408773e+20, 0.0, 2.0000000400817547e+20, 3.000000060122632e+20, 2.0000000400817547e+20, 0.0]}''')
WRONG = json.loads(r'''{"numpy": {"dtype": "float32", "shape": [6], "value": [-1.0000000200408773e+20, 0.0, 2.0000000400817547e+20, 3.000000060122632e+20, 2.0000000400817547e+20, 32768.0]}}''')
APIS = json.loads(r'''{"numpy": "numpy.fft.hfft", "scipy": "scipy.fft.hfft"}''')

def run_scipy():
    LIB = 'scipy'
    QNAME = 'scipy.fft.hfft'
    fn = resolve(QNAME)
