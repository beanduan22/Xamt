"""Minimal cross-library repro for bug_108 (lgamma/generic/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["jax", "mindspore"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, Infinity, Infinity], [1.389909267425537, 1.269892930984497, 2.4105467796325684]]}''')
WRONG = json.loads(r'''{"chainer": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, Infinity, -Infinity], [1.3899093866348267, 1.2698928117752075, 2.4105470180511475]]}}''')
APIS = json.loads(r'''{"chainer": "chainer.functions.lgamma", "jax": "jax.lax.lgamma"}''')

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.lax.lgamma'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.34056735038757324, -0.47251078486442566, 0.08592236787080765]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_chainer():
    LIB = 'chainer'
    QNAME = 'chainer.functions.lgamma'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.34056735038757324, -0.47251078486442566, 0.08592236787080765]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
