"""Minimal cross-library repro for bug_159 (aminmax/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''[{"dtype": "float32", "shape": [], "value": -0.11768058687448502}, {"dtype": "float32", "shape": [], "value": 0.21124573051929474}]''')
WRONG = json.loads(r'''{"mindspore": [{"dtype": "float32", "shape": [3], "value": [0.01884552091360092, -0.11768058687448502, -0.09371114522218704]}, {"dtype": "float32", "shape": [3], "value": [0.19023022055625916, 0.02389688976109028, 0.21124573051929474]}]}''')
APIS = json.loads(r'''{"mindspore": "mindspore.ops.aminmax", "torch": "torch.aminmax"}''')
...
def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.aminmax'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[1.0, 1.0, float("nan")], [2.0, 2.0, -0.0]], dtype=np.float32), LIB, QNAME), axis=None)
    return encode(normalize(result))
