"""Minimal cross-library repro for bug_178 (nextafter/generic/2)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["keras", "mindspore"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [3], "value": [NaN, Infinity, 0.0]}''')
WRONG = json.loads(r'''{"jax": {"dtype": "float32", "shape": [3], "value": [NaN, 3.4028234663852886e+38, 0.0]}}''')
APIS = json.loads(r'''{"jax": "jax.numpy.nextafter", "keras": "keras.ops.nextafter"}''')

def run_keras():
    LIB = 'keras'
    QNAME = 'keras.ops.nextafter'
    fn = resolve(QNAME)
