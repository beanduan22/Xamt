"""Minimal cross-library repro for bug_032 (argwhere/generic/component/1)."""
import importlib
import json
import os
...
WRONG = json.loads(r'''{"jax": {"dtype": "int32", "shape": [3, 2], "value": [[1, 0], [1, 1], [1, 2]]}, "mindspore": {"dtype": "int64", "shape": [3, 2], "value": [[1, 0], [1, 1], [1, 2]]}}''')
APIS = json.loads(r'''{"jax": "jax.numpy.argwhere", "mindspore": "mindspore.numpy.argwhere", "numpy": "numpy.argwhere"}''')

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.argwhere'
    fn = resolve(QNAME)
...
def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.numpy.argwhere'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[-0.0, 0.0, 1.401298464324817e-45], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
