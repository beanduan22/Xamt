"""Minimal cross-library repro for bug_062 (isclose/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["jax", "mindspore", "mxnet", "numpy", "paddle", "torch"]''')
EXPECTED = json.loads(r'''{"dtype": "bool", "shape": [2, 3], "value": [[false, true, true], [true, false, true]]}''')
WRONG = json.loads(r'''{"keras": {"dtype": "bool", "shape": [2, 3], "value": [[false, false, false], [true, false, true]]}}''')
APIS = json.loads(r'''{"keras": "keras.ops.isclose", "numpy": "numpy.isclose"}''')

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.isclose'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.14641617238521576, -0.046733640134334564, 0.16813381016254425]], dtype=np.float32), LIB, QNAME), tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.14641517400741577, -0.046732641756534576, 0.16813480854034424]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_keras():
    LIB = 'keras'
    QNAME = 'keras.ops.isclose'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.14641617238521576, -0.046733640134334564, 0.16813381016254425]], dtype=np.float32), LIB, QNAME), tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.14641517400741577, -0.046732641756534576, 0.16813480854034424]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
