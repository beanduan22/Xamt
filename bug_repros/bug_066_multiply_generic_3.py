"""Minimal cross-library repro for bug_066 (multiply/generic/3)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["mindspore", "mxnet"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, -Infinity], [0.18958938121795654, 0.2641873359680176, -1.0282534645966734e-07]]}''')
WRONG = json.loads(r'''{"tensorflow": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, NaN], [0.18958938121795654, 0.2641873359680176, -1.0282534645966734e-07]]}}''')
APIS = json.loads(r'''{"mxnet": "mxnet.numpy.multiply", "tensorflow": "tensorflow.math.multiply"}''')

def run_mxnet():
    LIB = 'mxnet'
    QNAME = 'mxnet.numpy.multiply'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.18958938121795654, 0.2641873359680176, -0.1028253436088562]], dtype=np.float32), LIB, QNAME), tensor(np.array([[-0.0, 0.0, 1.401298464324817e-45], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
