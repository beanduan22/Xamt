"""Minimal cross-library repro for bug_054 (ceil/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["chainer", "mindspore", "mxnet", "paddle", "torch"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[-0.0, 0.0, 1.0], [-1.0, 1.0, 1.0]]}''')
WRONG = json.loads(r'''{"jax": {"dtype": "float32", "shape": [2, 3], "value": [[-0.0, 0.0, 0.0], [-1.0, 1.0, 1.0]]}, "keras": {"dtype": "float32", "shape": [2, 3], "value": [[-0.0, 0.0, 0.0], [-1.0, 1.0, 1.0]]}, "tensorflow": {"dtype": "float32", "shape": [2, 3], "value": [[-0.0, 0.0, 0.0], [-1.0, 1.0, 1.0]]}}''')
APIS = json.loads(r'''{"jax": "jax.numpy.ceil", "keras": "keras.ops.ceil", "tensorflow": "tensorflow.math.ceil", "torch": "torch.ceil"}''')
