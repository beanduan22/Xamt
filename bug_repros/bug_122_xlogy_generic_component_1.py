"""Minimal cross-library repro for bug_122 (xlogy/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["tensorflow", "torch"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [3], "value": [0.0, 0.0, 0.0]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [3], "value": [NaN, NaN, -0.0]}}''')
APIS = json.loads(r'''{"mindspore": "mindspore.ops.function.xlogy", "torch": "torch.special.xlogy"}''')

def run_torch():
    LIB = 'torch'
    QNAME = 'torch.special.xlogy'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([-0.0, 0.0, 1.401298464324817e-45], dtype=np.float32), LIB, QNAME), tensor(np.array([-0.8061090111732483, -1.12245512008667, 0.3781912326812744], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.function.xlogy'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([-0.0, 0.0, 1.401298464324817e-45], dtype=np.float32), LIB, QNAME), tensor(np.array([-0.8061090111732483, -1.12245512008667, 0.3781912326812744], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
