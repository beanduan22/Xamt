"""Minimal cross-library repro for bug_015 (nanvar/generic/component/1)."""
import importlib
import json
import os
...
APIS = json.loads(r'''{"mindspore": "mindspore.numpy.nanvar", "numpy": "numpy.nanvar"}''')

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.nanvar'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.9399634003639221, -0.4968537986278534, 3.1000497341156006]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.numpy.nanvar'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.9399634003639221, -0.4968537986278534, 3.1000497341156006]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
