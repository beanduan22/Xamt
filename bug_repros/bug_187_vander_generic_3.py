"""Minimal cross-library repro for bug_187 (vander/generic/3)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["jax", "keras", "mindspore"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [3, 3], "value": [[Infinity, 9.999999680285692e+37, 1.0], [Infinity, -9.999999680285692e+37, 1.0], [0.0, 1.401298464324817e-45, 1.0]]}''')
WRONG = json.loads(r'''{"mxnet": {"dtype": "float64", "shape": [3, 3], "value": [[9.999999360571395e+75, 9.999999680285692e+37, 1.0], [9.999999360571395e+75, -9.999999680285692e+37, 1.0], [1.9636373861190906e-90, 1.401298464324817e-45, 1.0]]}, "numpy": {"dtype": "float64", "shape": [3, 3], "value": [[9.999999360571395e+75, 9.999999680285692e+37, 1.0], [9.999999360571395e+75, -9.999999680285692e+37, 1.0], [1.9636373861190906e-90, 1.401298464324817e-45, 1.0]]}}''')
APIS = json.loads(r'''{"jax": "jax.numpy.vander", "mxnet": "mxnet.numpy.vander", "numpy": "numpy.vander"}''')

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.numpy.vander'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([9.999999680285692e+37, -9.999999680285692e+37, 1.401298464324817e-45], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_mxnet():
    LIB = 'mxnet'
    QNAME = 'mxnet.numpy.vander'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([9.999999680285692e+37, -9.999999680285692e+37, 1.401298464324817e-45], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
