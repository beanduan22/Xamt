"""Minimal cross-library repro for bug_079 (softmax/nn/2)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["chainer", "jax", "scipy"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, NaN], [3.5585912883107085e-06, 0.0015719423536211252, 0.9984245300292969]]}''')
WRONG = json.loads(r'''{"keras": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, 0.0], [3.5585917430580594e-06, 0.0015719423536211252, 0.9984245300292969]]}, "tensorflow": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, 0.0], [3.5585917430580594e-06, 0.0015719423536211252, 0.9984245300292969]]}}''')
APIS = json.loads(r'''{"keras": "keras.ops.softmax", "scipy": "scipy.special.softmax", "tensorflow": "tensorflow.keras.activations.softmax"}''')

def run_scipy():
    LIB = 'scipy'
    QNAME = 'scipy.special.softmax'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-4.243289470672607, 1.8474136590957642, 8.30128002166748]], dtype=np.float32), LIB, QNAME), axis=1)
...
def run_tensorflow():
    LIB = 'tensorflow'
    QNAME = 'tensorflow.keras.activations.softmax'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-4.243289470672607, 1.8474136590957642, 8.30128002166748]], dtype=np.float32), LIB, QNAME), axis=1)
    return encode(normalize(result))
