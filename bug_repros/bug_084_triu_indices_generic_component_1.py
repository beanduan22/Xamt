"""Minimal cross-library repro for bug_084 (triu_indices/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''[{"dtype": "int32", "shape": [6], "value": [0, 0, 0, 1, 1, 2]}, {"dtype": "int32", "shape": [6], "value": [0, 1, 2, 1, 2, 2]}]''')
WRONG = json.loads(r'''{"paddle": {"dtype": "int64", "shape": [2, 6], "value": [[0, 0, 0, 1, 1, 2], [0, 1, 2, 1, 2, 2]]}}''')
APIS = json.loads(r'''{"numpy": "numpy.triu_indices", "paddle": "paddle.triu_indices"}''')

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.triu_indices'
    fn = resolve(QNAME)
    result = fn(3, 3)
