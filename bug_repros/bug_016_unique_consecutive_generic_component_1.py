"""Minimal cross-library repro for bug_016 (unique_consecutive/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [6], "value": [NaN, Infinity, -Infinity, -3.324608325958252, 0.9980002045631409, -0.3605535924434662]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [7], "value": [NaN, NaN, Infinity, -Infinity, -3.324608325958252, 0.9980002045631409, -0.3605535924434662]}, "paddle": {"dtype": "float32", "shape": [7], "value": [NaN, NaN, Infinity, -Infinity, -3.324608325958252, 0.9980002045631409, 0.0]}}''')
APIS = json.loads(r'''{"mindspore": "mindspore.ops.function.unique_consecutive", "paddle": "paddle.unique_consecutive", "torch": "torch.unique_consecutive"}''')

def run_torch():
    LIB = 'torch'
    QNAME = 'torch.unique_consecutive'
    fn = resolve(QNAME)
...
def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.function.unique_consecutive'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.0, -0.0, 1.0]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
...
def run_paddle():
    LIB = 'paddle'
    QNAME = 'paddle.unique_consecutive'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.0, -0.0, 1.0]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
