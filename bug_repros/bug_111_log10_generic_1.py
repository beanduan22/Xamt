"""Minimal cross-library repro for bug_111 (log10/generic/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[-Infinity, -Infinity, -Infinity], [NaN, 0.0, -6.0]]}''')
WRONG = json.loads(r'''{"chainer": {"dtype": "float32", "shape": [2, 3], "value": [[-Infinity, -Infinity, -44.85346984863281], [NaN, 0.0, -6.000000476837158]]}, "keras": {"dtype": "float32", "shape": [2, 3], "value": [[-Infinity, -Infinity, -45.15449905395508], [NaN, 0.0, -6.0]]}, "mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[-Infinity, -Infinity, -38.23080062866211], [NaN, 0.0, -6.0]]}}''')
APIS = json.loads(r'''{"chainer": "chainer.functions.log10", "jax": "jax.numpy.log10", "keras": "keras.ops.log10", "mindspore": "mindspore.ops.function.log10"}''')

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.numpy.log10'
    fn = resolve(QNAME)
...
def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.function.log10'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[-0.0, 0.0, 1.401298464324817e-45], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
