"""Minimal cross-library repro for bug_059 (hardswish/nn/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, Infinity, NaN], [-0.2272556722164154, 0.16052395105361938, -0.06130940839648247]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, Infinity, 0.0], [-0.2272556573152542, 0.1605239361524582, -0.06130940839648247]]}}''')
...
def run_jax():
    ...
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.5584767460823059, 0.2925243675708771, -0.1280876249074936]], dtype=np.float32), LIB, QNAME))
