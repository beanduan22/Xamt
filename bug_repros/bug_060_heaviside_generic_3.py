"""Minimal cross-library repro for bug_060 (heaviside/generic/3)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [3], "value": [0.5, 0.5, 1.0]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [3], "value": [0.5, 0.5, 0.5]}}''')
APIS = json.loads(r'''{"mindspore": "mindspore.numpy.heaviside", "paddle": "paddle.heaviside"}''')

def run_paddle():
    LIB = 'paddle'
    QNAME = 'paddle.heaviside'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([float("nan"), float("inf"), -0.0], dtype=np.float32), LIB, QNAME), tensor(np.array([0.5, 0.5, 0.5], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.numpy.heaviside'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([float("nan"), float("inf"), -0.0], dtype=np.float32), LIB, QNAME), tensor(np.array([0.5, 0.5, 0.5], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
