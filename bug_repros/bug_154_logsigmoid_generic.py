"""Minimal cross-library repro for bug_154 (logsigmoid/generic)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[-0.0, -1.0000000200408773e+20, -0.0], [-88.0, -0.6931471824645996, -0.6931471824645996]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[0.0, -Infinity, 0.0], [-Infinity, -0.6931471824645996, -0.6931471824645996]]}}''')
APIS = json.loads(r'''{"mindspore": "mindspore.ops.function.logsigmoid", "tensorflow": "tensorflow.math.log_sigmoid"}''')

def run_tensorflow():
    LIB = 'tensorflow'
    QNAME = 'tensorflow.math.log_sigmoid'
    fn = resolve(QNAME)
...
def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.function.logsigmoid'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[9.999999680285692e+37, -9.999999680285692e+37, 1.401298464324817e-45], [-1.401298464324817e-45, 0.0, -0.0]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
