"""Minimal cross-library repro for bug_124 (corrcoef/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["jax", "keras", "torch"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 2], "value": [[NaN, NaN], [NaN, 1.0]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 2], "value": [[-1.0, NaN], [NaN, 1.0]]}, "mxnet": {"dtype": "float64", "shape": [2, 2], "value": [[0.9999999999999998, 5.773502677158356e-07], [5.773502677158356e-07, 1.0]]}, "numpy": {"dtype": "float64", "shape": [2, 2], "value": [[0.9999999999999998, 5.773502677158356e-07], [5.773502677158356e-07, 1.0]]}}''')
APIS = json.loads(r'''{"jax": "jax.numpy.corrcoef", "mindspore": "mindspore.numpy.corrcoef", "mxnet": "mxnet.numpy.corrcoef", "numpy": "numpy.corrcoef"}''')

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.numpy.corrcoef'
    fn = resolve(QNAME)
...
def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.numpy.corrcoef'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[-0.0, 0.0, 1.401298464324817e-45], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
