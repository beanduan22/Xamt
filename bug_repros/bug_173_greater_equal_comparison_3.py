"""Minimal cross-library repro for bug_173 (greater_equal/comparison/3)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["mindspore", "tensorflow"]''')
EXPECTED = json.loads(r'''{"dtype": "bool", "shape": [2, 3], "value": [[true, false, true], [false, true, true]]}''')
WRONG = json.loads(r'''{"mxnet": {"dtype": "bool", "shape": [2, 3], "value": [[true, false, true], [false, true, false]]}}''')
APIS = json.loads(r'''{"mxnet": "mxnet.numpy.greater_equal", "tensorflow": "tensorflow.math.greater_equal"}''')

def run_tensorflow():
    LIB = 'tensorflow'
    QNAME = 'tensorflow.math.greater_equal'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[9.999999680285692e+37, -9.999999680285692e+37, 1.401298464324817e-45], [-1.401298464324817e-45, 0.0, -0.0]], dtype=np.float32), LIB, QNAME), tensor(np.array([[9.999999680285692e+37, 9.999999350456404e-39, -1.0], [1.0, -9.999999680285692e+37, 1.401298464324817e-45]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_mxnet():
    LIB = 'mxnet'
    QNAME = 'mxnet.numpy.greater_equal'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[9.999999680285692e+37, -9.999999680285692e+37, 1.401298464324817e-45], [-1.401298464324817e-45, 0.0, -0.0]], dtype=np.float32), LIB, QNAME), tensor(np.array([[9.999999680285692e+37, 9.999999350456404e-39, -1.0], [1.0, -9.999999680285692e+37, 1.401298464324817e-45]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
