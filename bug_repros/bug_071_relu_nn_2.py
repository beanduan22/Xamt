"""Minimal cross-library repro for bug_071 (relu/nn/2)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, Infinity, 0.0], [0.0, 6.798453330993652, 0.0]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[0.0, Infinity, 0.0], [0.0, 6.798453330993652, 0.0]]}, "paddle": {"dtype": "float32", "shape": [2, 3], "value": [[0.0, Infinity, 0.0], [0.0, 6.798453330993652, 0.0]]}}''')
APIS = json.loads(r'''{"mindspore": "mindspore.mint.nn.functional.relu", "paddle": "paddle.nn.functional.relu", "tensorflow": "tensorflow.nn.relu"}''')

def run_tensorflow():
    LIB = 'tensorflow'
    QNAME = 'tensorflow.nn.relu'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-2.0160984992980957, 6.798453330993652, -0.7453916668891907]], dtype=np.float32), LIB, QNAME))
