"""Minimal cross-library repro for bug_021 (mode/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["jax", "paddle"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2], "value": [-Infinity, 0.033594969660043716]}''')
WRONG = json.loads(r'''{"torch": {"dtype": "float32", "shape": [2], "value": [NaN, 0.033594969660043716]}}''')
APIS = json.loads(r'''{"jax": "jax.scipy.stats.mode", "torch": "torch.mode"}''')

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.scipy.stats.mode'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [1.3259097337722778, 0.0374438501894474, 0.033594969660043716]], dtype=np.float32), LIB, QNAME), axis=1)
...
def run_torch():
    ...
    QNAME = 'torch.mode'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [1.3259097337722778, 0.0374438501894474, 0.033594969660043716]], dtype=np.float32), LIB, QNAME), dim=1)
    return encode(normalize(result))
