"""Minimal cross-library repro for bug_036 (nonzero/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["mxnet", "numpy", "paddle", "torch"]''')
EXPECTED = json.loads(r'''{"dtype": "int64", "shape": [4, 2], "value": [[0, 2], [1, 0], [1, 1], [1, 2]]}''')
WRONG = json.loads(r'''{"jax": {"dtype": "int32", "shape": [3, 2], "value": [[1, 0], [1, 1], [1, 2]]}, "keras": {"dtype": "int32", "shape": [3, 2], "value": [[1, 0], [1, 1], [1, 2]]}, "mindspore": {"dtype": "int64", "shape": [3, 2], "value": [[1, 0], [1, 1], [1, 2]]}}''')
APIS = json.loads(r'''{"jax": "jax.numpy.nonzero", "keras": "keras.ops.nonzero", "mindspore": "mindspore.ops.function.nonzero", "numpy": "numpy.nonzero"}''')
...
def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.function.nonzero'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[-0.0, 0.0, 1.401298464324817e-45], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
