"""Minimal cross-library repro for bug_127 (percentile/generic/7)."""
import importlib
import json
import os
...
WRONG = json.loads(r'''{"mxnet": {"dtype": "float32", "shape": [], "value": -0.6650597453117371}}''')
APIS = json.loads(r'''{"jax": "jax.numpy.percentile", "mxnet": "mxnet.numpy.percentile"}''')

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.numpy.percentile'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.3182952404022217, -1.0118242502212524, -0.13589611649513245]], dtype=np.float32), LIB, QNAME), 50.0)
