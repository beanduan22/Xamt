"""Minimal cross-library repro for bug_100 (csd/signal/component/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''[{"dtype": "float64", "shape": [3], "value": [0.0, 0.25, 0.5]}, {"dtype": "float32", "shape": [3], "value": [NaN, NaN, NaN]}]''')
WRONG = json.loads(r'''{"jax": [{"dtype": "float32", "shape": [3], "value": [0.0, 0.25, 0.5]}, {"dtype": "complex64", "shape": [3], "value": [[NaN, NaN], [NaN, NaN], [NaN, NaN]]}]}''')
APIS = json.loads(r'''{"jax": "jax.scipy.signal.csd", "scipy": "scipy.signal.csd"}''')

def run_scipy():
    LIB = 'scipy'
    QNAME = 'scipy.signal.csd'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([float("nan"), float("inf"), -float("inf"), 4.665624618530273], dtype=np.float32), LIB, QNAME), tensor(np.array([float("nan"), float("inf"), -float("inf"), 4.665624618530273], dtype=np.float32), LIB, QNAME), nperseg=4)
    return encode(normalize(result))

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.scipy.signal.csd'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([float("nan"), float("inf"), -float("inf"), 4.665624618530273], dtype=np.float32), LIB, QNAME), tensor(np.array([float("nan"), float("inf"), -float("inf"), 4.665624618530273], dtype=np.float32), LIB, QNAME), nperseg=4)
    return encode(normalize(result))
