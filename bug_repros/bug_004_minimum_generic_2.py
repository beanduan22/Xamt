"""Minimal cross-library repro for bug_004 (minimum/generic/2)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, 0.0, -Infinity], [-1.1046050786972046, -0.6005114316940308, -0.3333442807197571]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[-0.0, 0.0, -Infinity], [-1.1046050786972046, -0.6005114316940308, -0.3333442807197571]]}}''')
APIS = json.loads(r'''{"keras": "keras.ops.minimum", "mindspore": "mindspore.ops.function.minimum"}''')

def run_keras():
    LIB = 'keras'
    QNAME = 'keras.ops.minimum'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-1.1046050786972046, -0.6005114316940308, -0.3333442807197571]], dtype=np.float32), LIB, QNAME), tensor(np.array([[-0.0, 0.0, 1.401298464324817e-45], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.function.minimum'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-1.1046050786972046, -0.6005114316940308, -0.3333442807197571]], dtype=np.float32), LIB, QNAME), tensor(np.array([[-0.0, 0.0, 1.401298464324817e-45], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
