"""Minimal cross-library repro for bug_162 (factorial/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[122828.6875, 0.9545602798461914, 221909188608.0], [8.254029813908759e+28, 87896608.0, 0.0]]}''')
WRONG = json.loads(r'''{}''')
APIS = json.loads(r'''{"jax": "jax.scipy.special.factorial", "scipy": "scipy.special.factorial"}''')

def run_scipy():
    LIB = 'scipy'
    QNAME = 'scipy.special.factorial'
    fn = resolve(QNAME)
...
def run_jax():
    LIB = 'jax'
    QNAME = 'jax.scipy.special.factorial'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[9.999999680285692e+37, -9.999999680285692e+37, 1.401298464324817e-45], [-1.401298464324817e-45, 0.0, -0.0]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
