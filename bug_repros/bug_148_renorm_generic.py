"""Minimal cross-library repro for bug_148 (renorm/generic)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["paddle", "torch"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[0.0, -0.0, 0.0], [-88.0, 9.999999682655225e-21, -9.999999682655225e-21]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[70.71067810058594, -70.71067810058594, 6.222539611568545e-17], [-88.0, 9.999999682655225e-21, -9.999999682655225e-21]]}}''')
APIS = json.loads(r'''{"mindspore": "mindspore.ops.function.renorm", "torch": "torch.renorm"}''')

def run_torch():
    LIB = 'torch'
    QNAME = 'torch.renorm'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[9.999999680285692e+37, -9.999999680285692e+37, 1.401298464324817e-45], [-1.401298464324817e-45, 0.0, -0.0]], dtype=np.float32), LIB, QNAME), 2, 0, 100.0)
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.function.renorm'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[9.999999680285692e+37, -9.999999680285692e+37, 1.401298464324817e-45], [-1.401298464324817e-45, 0.0, -0.0]], dtype=np.float32), LIB, QNAME), 2, 0, 100.0)
    return encode(normalize(result))
