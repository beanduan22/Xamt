"""Minimal cross-library repro for bug_171 (correlate/generic/3)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["mindspore", "mxnet", "numpy"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [1], "value": [1.999999761581421]}''')
WRONG = json.loads(r'''{"keras": {"dtype": "float32", "shape": [1], "value": [0.0]}}''')
APIS = json.loads(r'''{"keras": "keras.ops.correlate", "numpy": "numpy.correlate"}''')

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.correlate'
    fn = resolve(QNAME)
