"""Minimal cross-library repro for bug_153 (irfft/fft)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["jax", "paddle", "scipy", "torch"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [6], "value": [-1.666666773368904e+19, 0.0, 3.333333546737808e+19, 5.000000100204387e+19, 3.333333546737808e+19, 0.0]}''')
WRONG = json.loads(r'''{"numpy": {"dtype": "float32", "shape": [6], "value": [-1.666666773368904e+19, 0.0, 3.333333546737808e+19, 5.000000100204387e+19, 3.333333546737808e+19, 5461.33349609375]}}''')
APIS = json.loads(r'''{"numpy": "numpy.fft.irfft", "scipy": "scipy.fft.irfft"}''')

def run_scipy():
    LIB = 'scipy'
    QNAME = 'scipy.fft.irfft'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([9.999999680285692e+37, -9.999999680285692e+37, 1.401298464324817e-45, -0.0], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.fft.irfft'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([9.999999680285692e+37, -9.999999680285692e+37, 1.401298464324817e-45, -0.0], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
