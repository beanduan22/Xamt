"""Minimal cross-library repro for bug_017 (ihfft/fft/component/1)."""
import importlib
import json
import os
...
WRONG = json.loads(r'''{"jax": {"dtype": "complex64", "shape": [3], "value": [[NaN, NaN], [NaN, NaN], [NaN, NaN]]}}''')
APIS = json.loads(r'''{"jax": "jax.numpy.fft.ihfft", "numpy": "numpy.fft.ihfft"}''')

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.fft.ihfft'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([float("nan"), float("inf"), -float("inf"), 1.9586237668991089], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.numpy.fft.ihfft'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([float("nan"), float("inf"), -float("inf"), 1.9586237668991089], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
