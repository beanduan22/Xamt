"""Minimal cross-library repro for bug_027 (argmin/reduction/4)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "int32", "shape": [2], "value": [0, 2]}''')
WRONG = json.loads(r'''{"tensorflow": {"dtype": "int64", "shape": [2], "value": [2, 2]}}''')
APIS = json.loads(r'''{"numpy": "numpy.argmin", "tensorflow": "tensorflow.math.argmin"}''')

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.argmin'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.00815462227910757, -0.038530007004737854, -0.15682800114154816]], dtype=np.float32), LIB, QNAME), axis=1)
