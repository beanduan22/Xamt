"""Minimal cross-library repro for bug_010 (softshrink/nn/component/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, Infinity, -Infinity], [-1.9228339195251465, 1.0421700477600098, 0.6015607118606567]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[0.0, Infinity, -Infinity], [-1.9228339195251465, 1.0421700477600098, 0.6015607118606567]]}, "paddle": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, NaN], [-1.9228339195251465, 1.0421700477600098, 0.6015607118606567]]}}''')
APIS = json.loads(r'''{"mindspore": "mindspore.mint.nn.functional.softshrink", "paddle": "paddle.nn.functional.softshrink", "torch": "torch.nn.functional.softshrink"}''')

def run_torch():
    LIB = 'torch'
    QNAME = 'torch.nn.functional.softshrink'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-2.4228339195251465, 1.5421700477600098, 1.1015607118606567]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.mint.nn.functional.softshrink'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-2.4228339195251465, 1.5421700477600098, 1.1015607118606567]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
...
def run_paddle():
    LIB = 'paddle'
    QNAME = 'paddle.nn.functional.softshrink'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-2.4228339195251465, 1.5421700477600098, 1.1015607118606567]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
