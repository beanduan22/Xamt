"""Minimal cross-library repro for bug_037 (sort/generic/4)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["mxnet", "tensorflow"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, -Infinity, Infinity], [-7.590363502502441, 0.4991883635520935, 5.5815324783325195]]}''')
WRONG = json.loads(r'''{"jax": {"dtype": "float32", "shape": [2, 3], "value": [[-Infinity, Infinity, NaN], [-7.590363502502441, 0.4991883635520935, 5.5815324783325195]]}}''')
APIS = json.loads(r'''{"jax": "jax.lax.sort", "tensorflow": "tensorflow.sort"}''')

def run_tensorflow():
    LIB = 'tensorflow'
    QNAME = 'tensorflow.sort'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.4991883635520935, 5.5815324783325195, -7.590363502502441]], dtype=np.float32), LIB, QNAME))
