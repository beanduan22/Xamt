"""Minimal cross-library repro for bug_023 (argmax/reduction/4)."""
import importlib
import json
import os
...
WRONG = json.loads(r'''{"tensorflow": {"dtype": "int64", "shape": [2], "value": [1, 0]}}''')
APIS = json.loads(r'''{"numpy": "numpy.argmax", "tensorflow": "tensorflow.math.argmax"}''')

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.argmax'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.2070850431919098, -1.5477542877197266, -2.235914468765259]], dtype=np.float32), LIB, QNAME), axis=1)
    return encode(normalize(result))

def run_tensorflow():
    LIB = 'tensorflow'
    QNAME = 'tensorflow.math.argmax'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.2070850431919098, -1.5477542877197266, -2.235914468765259]], dtype=np.float32), LIB, QNAME), axis=1)
    return encode(normalize(result))
