"""Minimal cross-library repro for bug_105 (digamma/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["chainer", "mindspore", "paddle", "torch"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[Infinity, -Infinity, -Infinity], [NaN, -0.5772156715393066, -1000000.5625]]}''')
WRONG = json.loads(r'''{"jax": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, NaN], [NaN, -0.5772153735160828, -1000000.5625]]}, "tensorflow": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, NaN], [NaN, -0.5772159099578857, -1000000.5625]]}}''')
APIS = json.loads(r'''{"jax": "jax.scipy.special.digamma", "tensorflow": "tensorflow.math.digamma", "torch": "torch.special.digamma"}''')

def run_torch():
    LIB = 'torch'
    QNAME = 'torch.special.digamma'
    fn = resolve(QNAME)
...
def run_jax():
    LIB = 'jax'
    QNAME = 'jax.scipy.special.digamma'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[-0.0, 0.0, 1.401298464324817e-45], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
