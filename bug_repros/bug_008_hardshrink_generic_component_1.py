"""Minimal cross-library repro for bug_008 (hardshrink/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, Infinity, -Infinity], [0.5860238075256348, 0.0, 0.0]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[0.0, Infinity, -Infinity], [0.5860238075256348, 0.0, 0.0]]}}''')
APIS = json.loads(r'''{"mindspore": "mindspore.ops.function.hardshrink", "torch": "torch.hardshrink"}''')

def run_torch():
    LIB = 'torch'
    QNAME = 'torch.hardshrink'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.5860238075256348, -0.03903169557452202, 0.2753698527812958]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.function.hardshrink'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.5860238075256348, -0.03903169557452202, 0.2753698527812958]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
