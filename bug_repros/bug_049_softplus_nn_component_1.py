"""Minimal cross-library repro for bug_049 (softplus/nn/component/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, Infinity, 0.0], [0.6998663544654846, 0.596236526966095, 0.8535803556442261]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[Infinity, Infinity, 0.0], [0.6998662948608398, 0.596236526966095, 0.8535799384117126]]}}''')
APIS = json.loads(r'''{"mindspore": "mindspore.ops.function.softplus", "scipy": "scipy.special.softplus"}''')

def run_scipy():
    LIB = 'scipy'
    QNAME = 'scipy.special.softplus'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.013393441215157509, -0.2042306810617447, 0.2986507713794708]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.function.softplus'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.013393441215157509, -0.2042306810617447, 0.2986507713794708]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
