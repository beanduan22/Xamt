"""Minimal cross-library repro for bug_165 (topk/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["keras", "mindspore", "tensorflow"]''')
EXPECTED = json.loads(r'''[{"dtype": "float32", "shape": [2, 2], "value": [[-0.0, 0.0], [1.0, 9.999999974752427e-07]]}, {"dtype": "int32", "shape": [2, 2], "value": [[0, 1], [1, 2]]}]''')
WRONG = json.loads(r'''{"jax": [{"dtype": "float32", "shape": [2, 2], "value": [[1.401298464324817e-45, 0.0], [1.0, 9.999999974752427e-07]]}, {"dtype": "int32", "shape": [2, 2], "value": [[2, 1], [1, 2]]}], "paddle": [{"dtype": "float32", "shape": [2, 2], "value": [[1.401298464324817e-45, -0.0], [1.0, 9.999999974752427e-07]]}, {"dtype": "int64", "shape": [2, 2], "value": [[2, 0], [1, 2]]}], "torch": [{"dtype": "float32", "shape": [2, 2], "value": [[1.401298464324817e-45, -0.0], [1.0, 9.999999974752427e-07]]}, {"dtype": "int64", "shape": [2, 2], "value": [[2, 0], [1, 2]]}]}''')
APIS = json.loads(r'''{"jax": "jax.lax.top_k", "paddle": "paddle.topk", "tensorflow": "tensorflow.math.top_k", "torch": "torch.topk"}''')

def run_tensorflow():
    LIB = 'tensorflow'
    QNAME = 'tensorflow.math.top_k'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[1.0, 1.0, float("nan")], [2.0, 2.0, -0.0]], dtype=np.float32), LIB, QNAME), 2)
