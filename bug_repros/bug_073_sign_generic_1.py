"""Minimal cross-library repro for bug_073 (sign/generic/1)."""
import importlib
import json
import os
...
WRONG = json.loads(r'''{"chainer": {"dtype": "float32", "shape": [2, 3], "value": [[0.0, 0.0, 1.0], [-1.0, 1.0, 1.0]]}, "mindspore": {"dtype": "float32", "shape": [2, 3], "value": [[0.0, 0.0, 1.0], [-1.0, 1.0, 1.0]]}}''')
APIS = json.loads(r'''{"chainer": "chainer.functions.sign", "jax": "jax.numpy.sign", "mindspore": "mindspore.ops.function.sign"}''')
...
def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.ops.function.sign'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[-0.0, 0.0, 1.401298464324817e-45], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
