"""Minimal cross-library repro for bug_090 (cholesky/linalg/4)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["jax", "scipy"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 2], "value": [[2.246246099472046, 0.0], [-0.5446306467056274, 0.8483017086982727]]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [2, 2], "value": [[2.246246099472046, -0.5446306467056274], [0.0, 0.8483017086982727]]}}''')
APIS = json.loads(r'''{"mindspore": "mindspore.scipy.linalg.cholesky", "scipy": "scipy.linalg.cholesky"}''')

def run_scipy():
    LIB = 'scipy'
    QNAME = 'scipy.linalg.cholesky'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[5.045621395111084, -1.2233744859695435], [-1.2233744859695435, 1.0162383317947388]], dtype=np.float32), LIB, QNAME), lower=1)
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.scipy.linalg.cholesky'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[5.045621395111084, -1.2233744859695435], [-1.2233744859695435, 1.0162383317947388]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
