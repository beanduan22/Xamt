"""Minimal cross-library repro for bug_013 (nancumprod/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["jax", "mxnet", "numpy"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [6], "value": [1.0, Infinity, -Infinity, Infinity, Infinity, -Infinity]}''')
WRONG = json.loads(r'''{"keras": {"dtype": "float32", "shape": [6], "value": [1.0, 3.4028234663852886e+38, -Infinity, Infinity, Infinity, -Infinity]}}''')
APIS = json.loads(r'''{"keras": "keras.ops.nancumprod", "numpy": "numpy.nancumprod"}''')

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.nancumprod'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.3901340961456299, 0.25959521532058716, -0.8569552302360535]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_keras():
    LIB = 'keras'
    QNAME = 'keras.ops.nancumprod'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.3901340961456299, 0.25959521532058716, -0.8569552302360535]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
