"""Minimal cross-library repro for bug_024 (argmax/reduction/5)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "int64", "shape": [2], "value": [1, 0]}''')
WRONG = json.loads(r'''{"mxnet": {"dtype": "float32", "shape": [2], "value": [0.0, 0.0]}}''')
APIS = json.loads(r'''{"mxnet": "mxnet.ndarray.argmax", "paddle": "paddle.argmax"}''')

def run_paddle():
    LIB = 'paddle'
    QNAME = 'paddle.argmax'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.01071422453969717, -0.10324723273515701, -0.09497373551130295]], dtype=np.float32), LIB, QNAME), axis=1)
    return encode(normalize(result))

def run_mxnet():
    LIB = 'mxnet'
    QNAME = 'mxnet.ndarray.argmax'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.01071422453969717, -0.10324723273515701, -0.09497373551130295]], dtype=np.float32), LIB, QNAME), axis=1)
    return encode(normalize(result))
