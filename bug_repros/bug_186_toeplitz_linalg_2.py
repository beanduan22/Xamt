"""Minimal cross-library repro for bug_186 (toeplitz/linalg/2)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [3, 3], "value": [[NaN, Infinity, -0.0], [Infinity, NaN, Infinity], [-0.0, Infinity, NaN]]}''')
WRONG = json.loads(r'''{"jax": {"dtype": "float32", "shape": [3, 3], "value": [[NaN, NaN, NaN], [NaN, NaN, NaN], [NaN, NaN, NaN]]}}''')
APIS = json.loads(r'''{"jax": "jax.scipy.linalg.toeplitz", "scipy": "scipy.linalg.toeplitz"}''')

def run_scipy():
    LIB = 'scipy'
    QNAME = 'scipy.linalg.toeplitz'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([float("nan"), float("inf"), -0.0], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.scipy.linalg.toeplitz'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([float("nan"), float("inf"), -0.0], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
