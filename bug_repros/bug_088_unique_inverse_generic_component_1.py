"""Minimal cross-library repro for bug_088 (unique_inverse/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''[{"dtype": "float32", "shape": [5], "value": [-1.0, -0.0, 1.401298464324817e-45, 9.999999974752427e-07, 1.0]}, {"dtype": "int64", "shape": [2, 3], "value": [[1, 1, 2], [0, 4, 3]]}]''')
WRONG = json.loads(r'''{"jax": [{"dtype": "float32", "shape": [4], "value": [-1.0, -0.0, 9.999999974752427e-07, 1.0]}, {"dtype": "int32", "shape": [2, 3], "value": [[1, 1, 1], [0, 3, 2]]}]}''')
APIS = json.loads(r'''{"jax": "jax.numpy.unique_inverse", "numpy": "numpy.unique_inverse"}''')

def run_numpy():
    LIB = 'numpy'
    QNAME = 'numpy.unique_inverse'
    fn = resolve(QNAME)
...
def run_jax():
    LIB = 'jax'
    QNAME = 'jax.numpy.unique_inverse'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[-0.0, 0.0, 1.401298464324817e-45], [-1.0, 1.0, 9.999999974752427e-07]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
