"""Minimal cross-library repro for bug_076 (sign/generic/component/1)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[-0.0, 0.0, 0.0], [-1.0, 1.0, 1.0]]}''')
WRONG = json.loads(r'''{"chainer": {"dtype": "float32", "shape": [2, 3], "value": [[0.0, 0.0, 1.0], [-1.0, 1.0, 1.0]]}, "mxnet": {"dtype": "float32", "shape": [2, 3], "value": [[0.0, 0.0, 1.0], [-1.0, 1.0, 1.0]]}, "paddle": {"dtype": "float32", "shape": [2, 3], "value": [[0.0, 0.0, 1.0], [-1.0, 1.0, 1.0]]}, "torch": {"dtype": "float32", "shape": [2, 3], "value": [[0.0, 0.0, 1.0], [-1.0, 1.0, 1.0]]}}''')
APIS = json.loads(r'''{"chainer": "chainer.functions.sign", "jax": "jax.numpy.sign", "mxnet": "mxnet.ndarray.sign", "paddle": "paddle.sign", "torch": "torch.sign"}''')

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.numpy.sign'
    fn = resolve(QNAME)
