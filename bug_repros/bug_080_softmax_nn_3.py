"""Minimal cross-library repro for bug_080 (softmax/nn/3)."""
import importlib
import json
import os
...
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, 0.0], [0.23795250058174133, 0.33366093039512634, 0.42838653922080994]]}''')
WRONG = json.loads(r'''{"jax": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, NaN, NaN], [0.23795250058174133, 0.3336610198020935, 0.4283864498138428]]}}''')
APIS = json.loads(r'''{"jax": "jax.nn.softmax", "tensorflow": "tensorflow.math.softmax"}''')

def run_tensorflow():
    LIB = 'tensorflow'
    QNAME = 'tensorflow.math.softmax'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.11743292212486267, 0.22062155604362488, 0.4705217182636261]], dtype=np.float32), LIB, QNAME), axis=1)
    return encode(normalize(result))

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.nn.softmax'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-0.11743292212486267, 0.22062155604362488, 0.4705217182636261]], dtype=np.float32), LIB, QNAME), axis=1)
    return encode(normalize(result))
