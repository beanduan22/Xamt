"""Minimal cross-library repro for bug_005 (nancumsum/generic/3)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["keras"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [6], "value": [0.0, 3.4028234663852886e+38, 0.0, -4.2111430168151855, -17.07004737854004, -12.379446029663086]}''')
WRONG = json.loads(r'''{"mindspore": {"dtype": "float32", "shape": [6], "value": [0.0, Infinity, NaN, NaN, NaN, NaN]}}''')
APIS = json.loads(r'''{"keras": "keras.ops.nancumsum", "mindspore": "mindspore.numpy.nancumsum"}''')

def run_keras():
    LIB = 'keras'
    QNAME = 'keras.ops.nancumsum'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-4.2111430168151855, -12.858903884887695, 4.690601348876953]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_mindspore():
    LIB = 'mindspore'
    QNAME = 'mindspore.numpy.nancumsum'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [-4.2111430168151855, -12.858903884887695, 4.690601348876953]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
