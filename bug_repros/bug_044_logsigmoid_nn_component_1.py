"""Minimal cross-library repro for bug_044 (logsigmoid/nn/component/1)."""
import importlib
import json
import os
...
EXPECTED_LIBS = json.loads(r'''["jax", "torch"]''')
EXPECTED = json.loads(r'''{"dtype": "float32", "shape": [2, 3], "value": [[NaN, -0.0, -Infinity], [-0.35654228925704956, -0.5308101773262024, -0.7507658004760742]]}''')
WRONG = json.loads(r'''{"paddle": {"dtype": "float32", "shape": [2, 3], "value": [[NaN, -0.0, NaN], [-0.35654228925704956, -0.5308101773262024, -0.7507658004760742]]}}''')
APIS = json.loads(r'''{"jax": "jax.nn.log_sigmoid", "paddle": "paddle.nn.functional.log_sigmoid"}''')

def run_jax():
    LIB = 'jax'
    QNAME = 'jax.nn.log_sigmoid'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.8477401733398438, 0.3562331795692444, -0.11209741979837418]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))

def run_paddle():
    LIB = 'paddle'
    QNAME = 'paddle.nn.functional.log_sigmoid'
    fn = resolve(QNAME)
    result = fn(tensor(np.array([[float("nan"), float("inf"), -float("inf")], [0.8477401733398438, 0.3562331795692444, -0.11209741979837418]], dtype=np.float32), LIB, QNAME))
    return encode(normalize(result))
